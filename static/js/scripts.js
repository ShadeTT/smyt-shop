(function ($) {
    ...
    $(document).ready(function () {
        for (var i = 0; i <= 5; i += 1) {
            $('.rating[data-rating=' + i + ']').rateit({max: 5, step: 1, value: i, resetable: false, readonly: true});
        }
    });
...
// Price Slider
    ...
    $('.dropdown-toggle').on('click', function (e) {
        e.preventDefault();
        $(this).parent().find('.mega-menu').slideToggle(200);
    });
    $('.sub-menu_trigger').on('click', function (e) {
        e.preventDefault();
        $(this).next().slideToggle(200);
    });

    $('[data-name=add_to_compare]').click(function(e){
        var url =$(this).data('url'),
            id =$(this).data('id');
        $.post(url, {'id': id}, function(data){});
    });
    $('[data-name=remove_from_compare]').click(function(e){
        var $this = $(this),
            event = e,
            url =$this.data('url'),
            id =$this.data('id');
        $.post(url, {'id': id, 'delete': true}, function(data){
            if(data['success']){
                var ndx = $this.parent().index() + 1;
                $this.parents('table').find("td").remove(":nth-child(" + ndx + ")");
            }
        });
    });
})(jQuery);